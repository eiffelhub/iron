...
--						end
--						s.append (" <a style=%"color: red;%" href=%"" + iron.package_map_web_page (iron_version (req), l_package, c.item) + "?_method=DELETE%">DEL</a>")
--						s.append ("</li>%N")
--					end
--					s.append ("</ul>")
--				end
		...
		do
			if attached package_version_from_id_path_parameter (req, "id") as l_package	then
				if has_permission_to_modify_package_version (req, l_package) then
					...
					if l_path /= Void then
						l_repo_url := req.absolute_script_url ("/" + l_package.version.value)
						...
						if l_path.has (':') or l_path.has_substring ("..") then
							-- invalid path
							err := True
							m.add_error_message ("Invalid map parameter (should not contain ':' or '..' )")
						else
--							if l_path.starts_with ("/") then
--								l_path.remove_head (1)
--							end
							if attached iron.database.package_by_path (l_package.version, l_path) as p_curr then
								m.add_error_message ("Already associated to " + m.encoded_string (p_curr.human_identifier))
								err := True
							else
								iron.database.associate_package_with_path (l_package, l_path)
								if iron.database.package_by_path (l_package.version, l_path) ~ l_package then
									-- succeed
									m.add_normal_message ("association created.")
								else
									-- failure
									m.add_error_message ("association creation failed.")
									err := True
								end
							end
						end
					else
						m.add_error_message ("Missing map parameter.")
					end
					res.send (m)
				else
					...
					then
						...
						if iron.database.package_by_path (iron_version (req), l_path) ~ l_package then
							...
							if iron.database.package_by_path (iron_version (req), l_path) = Void then
								-- succeed
								m.add_normal_message ("association removed.")
							else
								-- failure
								m.add_error_message ("association removal failed.")
							end
						else
							m.add_warning_message ("Not associated.")
						end
					else
						m.add_error_message ("Missing map parameter.")
					end
